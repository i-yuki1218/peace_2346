...
point=str(count)
#文字に変換した数字をばらばらにする
#map関数を使うことで、リストのオブジェクトに含まれる各要素に対して変更を加えて別のオブジェクトを生成できる。
array=list(map(int,point))
#len＝要素数。二桁以上なら、さっき分解した数字をaとbに分解する
if len(array) >= 2:
    a,b = array
...
if st.button('結果を見る'):
    if k == 1:
         st.write('素晴らしい行動力の持ち主で、頭の回転も速く周りからよく頼られるでしょう。',
             'ですが、はやとちりや周りを置いて行ってしまうことには注意が必要かも...',)
         st.image('girl1.png',use_column_width=True)
    
    elif k == 2:
        st.write('さっぱりした性格で、必要に応じて立場を考え行動が出来るでしょう。',
             'ですが、執着がないことには要注意。異性からの好意を得にくいかも...',)
        ...
        st.write('真面目で曲がったことが嫌いなあなたは、人にだまされることが少ないでしょう。',
             'しかし、突き進みすぎて周りが見えなくなってしまうことがあるかも...',)
        st.image('girl5.png',use_column_width=True)
    
    
    elif k == 6:
        st.write('社交的で、あたまの回転も速く、相手を理解する能力も高いので、話すことで励まされる人がたくさんいるはずです。',
        ...
        st.write('穏やかな性格で純粋なあなた。嘘や計算が嫌いで、他人に損得勘定抜きで接するところが魅力です。',
        ...
        st.write('判断力があり、最後まで諦めないあなたは、目標に向かって一直線に努力できるタイプです。',
             'しかし、周りが見えなくなってしまうこともあるので注意が必要。',)
        st.image('girl22.png',use_column_width=True)
    
    elif k == 33:
        st.write('天才肌の持ち主です！',
             '仕事では、大成功か大失敗の両極端になりがちで、周りの環境次第で人生が変化するタイプです。',)
